...
namespace labutils
{
	...
	// make_vulkan_window()
	std::pair<VulkanWindow, VkPhysicalDeviceLimits> make_vulkan_window()
	{
		VulkanWindow ret;
        ...
        //Initialise GLFW and make sure this GLFW supports Vulkan.
        //Note: this assumes that we will not create multiple windows that exist concurrently. If multiple windows are
        // to be used, the glfwInit() and glfwTerminate() (see destructor) calls should be moved elsewhere.
        ...
        //GLFW my require a number of instance extensions.
        //GLFW returns a bunch of pointers to strings; however, GLFW manages these internally so we must not free them
        //ourselves. GLFW guarantees that the strings remain valid until GLFW terminates.
        ...
        for(std::uint32_t  i =0; i<reqExtCount; ++i){
            if(!supportedExtensions.count(requiredExt[i])){
                throw lut::Error("GLFW/Vulkan: required instance extension %s is not supported", requiredExt[i]);
            }
...
#		endif // ~ debug builds
        ...
        if(ret.swapViews.size() == 0 && ret.swapImages.size() == 0){
        }
		// Done
		return std::make_pair(std::move(ret),limits);
	}
...
//        Remember old format & extents
        ...
        try {
            ...
            //Put back the old swapchain handle into the VulkanWindow, this ensures that the old swap chain is
...
namespace
{
	...
	{
        std::uint32_t  formatN;

        if(auto const res = vkGetPhysicalDeviceSurfaceFormatsKHR(aPhysicalDev, aSurface, &formatN, nullptr); VK_SUCCESS != res){
            throw lut::Error("Unable to get number of VkSurfaceFormats\n"
                             "vkGetPhysicalDeviceSurfaceFormatsKHR returned %s", lut::to_string(res).c_str());
            return{};
        }

        std::vector<VkSurfaceFormatKHR> formats(formatN);

        if(auto const res = vkGetPhysicalDeviceSurfaceFormatsKHR(aPhysicalDev, aSurface, &formatN, formats.data()); VK_SUCCESS != res){
            throw lut::Error("Unable to get VkSurfaceFormats properties\n"
                             "vkGetPhysicalDeviceSurfaceFormatsKHR returned %s", lut::to_string(res).c_str());
            return {};
        }
	...
	{
        std::uint32_t presentN;

        if(auto const res = vkGetPhysicalDeviceSurfacePresentModesKHR(aPhysicalDev,aSurface, &presentN, nullptr); VK_SUCCESS != res){
            throw lut::Error("Unable to enumerate VkPresentModeKHR\n"
                             "vkGetPhysicalDeviceSurfacePresentModesKHR returned %s", lut::to_string(res).c_str());
            return {};
        }

        std::vector<VkPresentModeKHR> presentModes(presentN);

        if(auto const res = vkGetPhysicalDeviceSurfacePresentModesKHR(aPhysicalDev,aSurface, &presentN, presentModes.data() ); VK_SUCCESS != res){
            throw lut::Error("Unable to get VkPresentModeKHR properties\n"
                             "vkGetPhysicalDeviceSurfacePresentModesKHR returned %s", lut::to_string(res).c_str());
            return {};
        }
	...
	std::tuple<VkSwapchainKHR,VkFormat,VkExtent2D> create_swapchain( VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface, VkDevice aDevice, GLFWwindow* aWindow, std::vector<std::uint32_t> const& aQueueFamilyIndices, VkSwapchainKHR aOldSwapchain )
	{
        ...
        if(auto const res = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(aPhysicalDev, aSurface, &caps); VK_SUCCESS != res){
            throw lut::Error("Unable to get surface capabilities\n"
                             "vkGetPhyscalDeviceSurfaceCapabilitiesKHR() returned %s", lut::to_string(res).c_str());
        }
        ...
        if(aQueueFamilyIndices.size() <=1 ){
            ...
            //Multiple queues may access this resource. There are 2 options: SHARING_MODE_CONCURRENT allows access from
            //multiple quques without transferring ownership; EXCLUSIVE would require explicit ownership transfers,
            //which we are avoiding for now. EXCLUSIVE may result in better performance than concurrent.
	...
	void get_swapchain_images( VkDevice aDevice, VkSwapchainKHR aSwapchain, std::vector<VkImage>& aImages )
	{
		...
		// get swapchain image handles with
        std::uint32_t swapchainImageN;
        if(auto res = vkGetSwapchainImagesKHR(aDevice, aSwapchain, &swapchainImageN, nullptr); VK_SUCCESS !=  res){
            throw lut::Error("Cannot enumerate vkSwapchainImages"
                             "vkGetSwapchainImagesKHR returned %s", lut::to_string(res).c_str());
        }

        aImages.resize(swapchainImageN);
        if(auto res = vkGetSwapchainImagesKHR(aDevice, aSwapchain, &swapchainImageN, aImages.data() ); VK_SUCCESS !=  res){
            throw lut::Error("Cannot get detail from vkSwapchainImages"
                             "vkGetSwapchainImagesKHR returned %s", lut::to_string(res).c_str());
        }
...
namespace labutils{
    bool anisotropic_filtering(VkPhysicalDevice device){
        VkPhysicalDeviceProperties props;
        ...
        //Note: the encoding of the driver version is not standardised.
...
namespace
{
	...
	float score_device( VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface )
	{
        ...
        if(!find_queue_family(aPhysicalDev, 0, aSurface)){
            std::fprintf(stderr, "Infor: discarding device '%s': can't present to surface\n", props.deviceName);
            return -1.f;
        }
