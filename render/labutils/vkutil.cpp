...
namespace labutils
{
    DescriptorPool create_imgui_descriptor_pool(VulkanContext const& aContext) {
        ...
        if(auto const res = vkCreateDescriptorPool(aContext.device, &pool_info, nullptr, &imguiPool);
                VK_SUCCESS != res){
            throw Error("Unable to create descriptor pool\n"
                        "vkCreateDescriptorPool() returned %s", to_string(res).c_str());
        }
        return DescriptorPool (aContext.device, imguiPool);
    }
	...
	CommandPool create_command_pool( VulkanContext const& aContext, VkCommandPoolCreateFlags aFlags )
	{
        ...
        if(auto const res = vkCreateCommandPool(aContext.device, &poolInfo, nullptr, &cpool); VK_SUCCESS != res){
            throw Error("Unable to create commad pool\n"
	...
	{
        VkCommandBufferAllocateInfo cbuInfo{};
        cbuInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
        cbuInfo.commandPool = aCmdPool;
        cbuInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
        cbuInfo.commandBufferCount = 1;

        VkCommandBuffer cbuff = VK_NULL_HANDLE;
        if(auto const res = vkAllocateCommandBuffers(aContext.device, &cbuInfo, &cbuff); VK_SUCCESS != res){
    ...
    void buffer_barrier(VkCommandBuffer aCmdBuff, VkBuffer aBuffer, VkAccessFlags aSrcAccessMask,
                         ...
                         VkPipelineStageFlags aDstStageMask, VkDeviceSize aSize,
    ...
    DescriptorPool create_descriptor_pool(VulkanContext const& aContext,
    ...
    void image_barrier (VkCommandBuffer aCmdBuff, VkImage aImage, VkAccessFlags aSrcAccessMask, VkAccessFlags aDstAccessMask,
        ...
        vkCmdPipelineBarrier(aCmdBuff, aSrcStageMask, aDstStageMask,
                             0,0, nullptr, 0,
                             nullptr, 1 , &ibarrier);

    }

    Sampler create_default_sampler( VulkanContext const& aContext, bool& anisotropicFiltering, float& maxAnisotropy ){
    ...
    samplerInfo.anisotropyEnable = anisotropicFiltering; //Enable anisotropic filtering if available
