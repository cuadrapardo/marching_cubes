...
namespace labutils
{
	...
	{
        //Flip images vertically by defauly. Vulkan expects the first scanline to be the bottom-most scanline.
        //PNG et al intstead define the first scanline to be the top-most one
        ...
        auto staging = create_buffer(aAllocator, sizeInBytes,
        ...
        VK_SUCCESS != res){
            throw Error("Mapping memory for writing\n"
                        "vmaMapMemrory() returned %s", to_string(res).c_str());
        }
        ...
        //When copying data to the image, the image's layout MUST be TRANSFER_DAST_OPTIMAL
        //Current image layout is undefined.
        ...
        for(std::uint32_t level = 1; level < mipLevels; ++level){
            ...
            //Level 0 is the base level that we initialise before the loop.
            VkImageBlit blit{};
            ...
            //Transtion mip level to TRANSFER_SRC_OPTIMAL for next iteration.
            //Technically this is unnecessary for the last mip level, but transitioning it as well simplifies the final
            //barrier following the loop.
            image_barrier( cbuff, ret.image,
                           VK_ACCESS_TRANSFER_WRITE_BIT,
                           VK_ACCESS_TRANSFER_READ_BIT,
                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        ...
        //Whole image is currently in the TRANSFER_SRC_OPTIMAL layout. To use the image as a texture from which we sample,
        //it must be in the SHADER_READ_ONLY_OPTIMAL_LAYOUT
        image_barrier(cbuff, ret.image,
                      VK_ACCESS_TRANSFER_READ_BIT,
        ...
        //Submit command buffer and wait for commands to complete. Commands must have completed before we can destroy the
        //temporary resources, such as the staging buffers.
    ...
    if(auto const res = vkWaitForFences(aContext.device, 1, &uploadComplete.handle, VK_TRUE, std::numeric_limits<std::uint64_t>::max() );
       ...
       throw Error("Waiting for upload to complete\n"
        ...
        //Most temporary resource are destroyed automaticall through their destructors. However, the
        //command buffer must be freed manually.
